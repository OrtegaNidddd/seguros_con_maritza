import { ChevronRight, GraduationCap, Heart, Stethoscope, AlertTriangle, Building2, CarFront, PiggyBank, HomeIcon } from "lucide-react"
...
const services = [
  ...
  { icon: Heart, title: "Seguros de Vida", desc: "Proteges el bienestar de toda la familia", color: "from-pink-200 to-pink-300", textColor: "text-pink-800", borderColor: "border-pink-800", featured: false },
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-2 gap-6 justify-items-center">
          {services.map((service) => (
            <div
              key={service.title}
              className={`group relative overflow-hidden rounded-xl border-2 ${service.borderColor ?? "border-border"} transition-all duration-300 ease-out cursor-pointer h-[13.5rem] w-11/12 mx-auto flex flex-col justify-between p-5 hover:-translate-y-2 hover:shadow-2xl ${service.featured ? "bg-gradient-to-br from-yellow-200/80 to-amber-300/80" : "bg-gradient-to-br " + service.color}`}
            >
